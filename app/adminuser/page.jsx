...
function PasswordGate({ onAuthenticated, pushToast }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: "Not Authenticated" }));
        pushToast({ variant: "error", message: data.error || "Not Authenticated" });
        return;
      }
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-20">
      <div className="w-full max-w-md rounded-2xl bg-card/80 p-8 shadow-2xl">
        <div className="mb-8 text-center">
          <p className="text-sm font-semibold uppercase tracking-widest text-accent">Admin Portal</p>
...
function AddWorkTab({ onCreated, pushToast }) {
  ...
  const handleGalleryChange = (id, file) => {
    setGalleryFields((prev) =>
      prev.map((entry) => {
        if (entry.id === id) {
          if (entry.file?._previewUrl) URL.revokeObjectURL(entry.file._previewUrl);
          return { ...entry, file };
        }
        return entry;
      })
  ...
  const handleRemoveGalleryField = (id) => {
    setGalleryFields((prev) => {
      const next = prev.length === 1 ? prev : prev.filter((entry) => entry.id !== id);
      const removed = prev.find((entry) => entry.id === id);
      if (removed?.file?._previewUrl) URL.revokeObjectURL(removed.file._previewUrl);
      return next;
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (!name.trim() || !description.trim()) {
      pushToast({ variant: "error", message: "Name and description are required" });
      return;
    }
    if (!titleImage) {
    ...
    try {
      const [titleImageUrl, galleryImageUrls] = await Promise.all([
        ...
        Promise.all(galleryFiles.map((file) => uploadImageToCloud(file, "works/gallery"))),
      ]);
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: "Failed to save work" }));
      ...
      pushToast({ variant: "error", message: error.message || "Failed to add work" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 rounded-3xl border border-border bg-card/80 px-6 py-7 shadow-xl text-sm">
      <div>
        <label className="mb-2 block text-xs font-semibold uppercase tracking-wide">Name of the Work</label>
        <input
          type="text"
          value={name}
      ...
      <div>
        <label className="mb-2 block text-xs font-semibold uppercase tracking-wide">Description</label>
        <textarea
          value={description}
      ...
      <div>
        <label className="mb-2 block text-xs font-semibold uppercase tracking-wide">Title Image</label>
      ...
      <div>
        <div className="flex items-center justify-between">
          <label className="mb-2 block text-xs font-semibold uppercase tracking-wide">Additional Images</label>
          <button
            type="button"
          ...
          {galleryWithPreview.map((field) => (
            ...
            >
              <input
                type="file"
                accept="image/*"
                onChange={(event) => handleGalleryChange(field.id, event.target.files?.[0] || null)}
                disabled={submitting}
              />
              <div className="flex flex-1 items-center justify-between gap-3 text-xs text-muted-foreground">
                <div className="flex items-center gap-3">
...
function ManageWorkTab({ refreshKey, pushToast, onEdit, onDelete }) {
  ...
  const fetchWorks = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: "Failed to load" }));
      ...
      pushToast({ variant: "error", message: err.message || "Unable to load works" });
    } finally {
      setLoading(false);
    }
  }, [pushToast]);
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        <table className="min-w-full divide-y divide-border text-sm">
          ...
          <tbody className="divide-y divide-border bg-card">
            {works.map((work) => (
              <tr key={work.id} className="hover:bg-secondary/40">
                <td className="px-6 py-4 font-semibold text-foreground">{work.name}</td>
                <td className="px-6 py-4 text-muted-foreground">
                  <p className="line-clamp-2">{work.description}</p>
                </td>
                <td className="px-6 py-4 text-muted-foreground">
                  <p className="text-xs">Title + {work.galleryImageUrls?.length || 0} gallery</p>
                </td>
                <td className="px-6 py-4 text-muted-foreground">
...
export default function AdminDashboardPage() {
  ...
  const handleDelete = async (work) => {
    ...
    try {
      ...
      pushToast({ variant: "error", message: error.message || "Failed to delete work" });
    }
  };
  ...
  const handleModalSubmit = async (event) => {
    ...
    try {
      ...
      if (editingWork.pendingTitleFile) {
        titleImageUrl = await uploadImageToCloud(editingWork.pendingTitleFile, "works/title");
      }

      const galleryImageUrls = [...(editingWork.galleryImageUrls || [])];
      if (editingWork.pendingGallery?.length) {
        const uploaded = await Promise.all(
          editingWork.pendingGallery.map((file) => uploadImageToCloud(file, "works/gallery"))
        );
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: "Failed to update" }));
      ...
      pushToast({ variant: "error", message: error.message || "Failed to update work" });
    } finally {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-background via-secondary/30 to-background px-3 py-8 text-foreground sm:px-6">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 lg:flex-row">
        ...
        {!authenticated ? (
          <PasswordGate onAuthenticated={handleAuthenticated} pushToast={pushToast} />
        ) : (
          <>
            <TabSidebar activeTab={activeTab} onChange={setActiveTab} />
            <section className="flex-1 space-y-6">
              <header className="rounded-3xl border border-border bg-card/80 px-6 py-6 shadow-xl text-sm">
                ...
                <p className="mt-2 text-muted-foreground">
                  Manage showcased work with a smoother, more compact experience.
                </p>
              </header>
              {activeTab === "add" ? (
                <AddWorkTab onCreated={handleWorkCreated} pushToast={pushToast} />
              ) : (
                <ManageWorkTab
      ...
      {showModal && editingWork && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-lg rounded-3xl border border-border bg-card p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Edit Work</p>
            ...
            <form className="space-y-4 text-sm" onSubmit={handleModalSubmit}>
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide">Name</label>
                <input
                  value={editingWork.name}
                  onChange={(e) => setEditingWork((prev) => ({ ...prev, name: e.target.value }))}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setEditingWork((prev) => ({ ...prev, description: e.target.value }))}
                  rows={3}
                  className="w-full rounded-xl border border-border bg-background px-3 py-2"
                  required
                />
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide">Title Image</label>
                <div className="flex items-center gap-4">
                  <img
                    src={editingWork.pendingTitleFile ? URL.createObjectURL(editingWork.pendingTitleFile) : editingWork.titleImageUrl || editingWork.image}
                    alt="Title"
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingWork((prev) => ({
                          ...
                          titleImageUrl: e.target.files?.[0] ? prev.titleImageUrl : prev.titleImageUrl,
                        }))
                      }
                    />
                  </label>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-xs font-semibold uppercase tracking-wide">Gallery Images</label>
                      ...
                      onChange={(e) =>
                        setEditingWork((prev) => ({
                          ...
                          pendingGallery: e.target.files?.[0]
                            ? [...(prev.pendingGallery || []), e.target.files[0]]
                            : prev.pendingGallery,
                        }))
                      }
                    />
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(editingWork.galleryImageUrls || []).map((url, index) => (
                    <div key={url + index} className="relative">
                      <img src={url} alt="Gallery" className="h-24 w-full rounded-xl object-cover border border-border" />
                      <button
                        type="button"
                        ...
                        onClick={() =>
                          setEditingWork((prev) => ({
                            ...prev,
                            galleryImageUrls: prev.galleryImageUrls.filter((_, idx) => idx !== index),
                          }))
                        }
                      >
                        ✕
                    ...
                    <div key={file.name + index} className="relative">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setEditingWork((prev) => ({
                            ...prev,
                            pendingGallery: prev.pendingGallery.filter((_, idx) => idx !== index),
                          }))
                        }
                      >
                        ✕
